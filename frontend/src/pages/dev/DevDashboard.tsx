...
export function DevDashboard() {
  ...
  const counts = useQuery({
    queryKey: ['admin-table-counts', mode],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-display font-bold flex items-center gap-2">
            <Terminal className="size-8" /> Admin Console
          </h1>
          <p className="text-slate-500">
            Quan tri he thong voi backend hien tai: <code className="font-mono bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">{mode}</code>
          </p>
        </div>
        <Button
          onClick={() => resetMutation.mutate()}
          disabled
          title="Luan backend does not expose reset/reseed"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-sm text-slate-500">Tong so bang</p>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-sm text-slate-500">Tong ban ghi</p>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <p className="text-3xl font-display font-bold">Luan</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Thong ke theo bang</CardTitle>
          <CardDescription>So ban ghi hien co trong moi bang cua backend Luan</CardDescription>
        </CardHeader>
        <CardContent>
