...
export const Ttcn = sequelize.define('ttcn', {
  ...
  Ho: strCol(), Ten: strCol(),
  vaiTro: strCol(), matKhau: strCol(),
...
export const SinhVien = sequelize.define('sinhVien', {
  ...
  GPA: { type: DataTypes.FLOAT, validate: { min: 0, max: 4.0 } },
  CPA: { type: DataTypes.FLOAT, validate: { min: 0, max: 4.0 } },
}, common);
...
export const ChuyenNganh = sequelize.define('chuyenNganh', {
  ...
  tenChuyenNganh: strCol(), soTinChi: DataTypes.INTEGER, dieuKien: txtCol(),
}, common);
...
export const SinhVien_NghienCuu = sequelize.define('sinhVien_NghienCuu', {
  ...
  MCB: idCol(),
  ngayThamGia: DataTypes.DATEONLY, trangThai: strCol(), moTa: txtCol(),
}, common);
...
export const DoAnTN = sequelize.define('doAnTN', {
  ...
  bacDoAn: strCol(), moTa: txtCol(), dinhKem: txtCol(),
}, common);

export const SinhVien_DoAnTN = sequelize.define('sinhVien_DoAnTN', {
  MSV: { ...idCol(), primaryKey: true }, maDoAn: { ...idCol(), primaryKey: true },
  ngayDangKy: DataTypes.DATEONLY, trangThai: strCol(),
}, common);
...
export const SinhVien_DuHoc = sequelize.define('sinhVien_DuHoc', {
  ...
  ngayDangKy: DataTypes.DATEONLY, phanTram: DataTypes.DECIMAL(5, 2), trangThai: strCol(),
}, common);
...
export const MODELS: Record<TableName, ModelStatic<Model>> = {
  ...
  mon: Mon, monDaoTao: MonDaoTao, bangDiem: BangDiem,
...
// Lớp tín chỉ -> Môn & Cán bộ
LopTinChi.belongsTo(Mon, { foreignKey: 'maMon' });
LopTinChi.belongsTo(CanBo, { foreignKey: 'MCB' });

// Cán bộ -> TTCN
CanBo.belongsTo(Ttcn, { foreignKey: 'CCCD', targetKey: 'CCCD' });

// Sinh viên -> TTCN & Khoa
SinhVien.belongsTo(Ttcn, { foreignKey: 'CCCD', targetKey: 'CCCD' });
SinhVien.belongsTo(Khoa, { foreignKey: 'maKhoa' });

// Đề tài nghiên cứu -> Sinh viên tham gia
NghienCuu.hasMany(SinhVien_NghienCuu, { foreignKey: 'maDeTai' });
SinhVien_NghienCuu.belongsTo(NghienCuu, { foreignKey: 'maDeTai' });
SinhVien_NghienCuu.belongsTo(SinhVien, { foreignKey: 'MSV' });
SinhVien_NghienCuu.belongsTo(CanBo, { foreignKey: 'MCB' });

// Đồ án -> Sinh viên (qua bảng tham gia)
DoAnTN.hasMany(SinhVien_DoAnTN, { foreignKey: 'maDoAn' });
SinhVien_DoAnTN.belongsTo(DoAnTN, { foreignKey: 'maDoAn' });
SinhVien_DoAnTN.belongsTo(SinhVien, { foreignKey: 'MSV' });
