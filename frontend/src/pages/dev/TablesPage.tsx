...
export function TablesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin-table', table, search, page],
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['admin-table', table] });

  const createMut = useMutation({
    mutationFn: (data: Row) => repositories.generic.create(table, data),
    onSuccess: () => { toast.success('Da tao'); setCreateOpen(false); invalidate(); },
    onError: (e) => toast.error(e instanceof Error ? e.message : 'Loi tao'),
  });

  const updateMut = useMutation({
    mutationFn: ({ pk, data }: { pk: Row; data: Row }) => repositories.generic.update(table, pk, data),
    onSuccess: () => { toast.success('Da cap nhat'); setEditRow(null); invalidate(); },
    onError: (e) => toast.error(e instanceof Error ? e.message : 'Loi cap nhat'),
  });

  const deleteMut = useMutation({
    mutationFn: (pk: Row) => repositories.generic.delete(table, pk),
    onSuccess: () => { toast.success('Da xoa'); invalidate(); },
    onError: (e) => toast.error(e instanceof Error ? e.message : 'Loi xoa'),
  });
    ...
    <div className="flex gap-4 h-[calc(100vh-8rem)]">
      <aside className="w-56 shrink-0">
        <Card className="h-full">
          <CardHeader><CardTitle className="text-sm">{ALL_TABLES.length} bang</CardTitle></CardHeader>
        ...
        <Card className="h-full flex flex-col">
          <CardHeader className="flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500">{total} ban ghi - PK: {meta.primaryKeys.join(', ')}</p>
            </div>
            <div className="flex gap-2 items-center">
              <Input placeholder="Tim kiem..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} className="max-w-xs" />
              ...
              <Button onClick={() => setCreateOpen(true)}><Plus className="size-4" /> Them</Button>
            </div>
          </CardHeader>
          <CardContent className="flex-1 overflow-auto">
            {isLoading ? (
              ...
              <p className="text-slate-500">Khong co du lieu</p>
            ) : (
                    ...
                    <tr key={i} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-900/30 group">
                      ...
                      <td className="py-1 px-3 text-right opacity-0 group-hover:opacity-100 transition">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => {
                          if (confirm('Xoa ban ghi nay?')) deleteMut.mutate(getPK(row));
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title={`Tao ${table}`} size="lg">
        <ZodForm
          schema={meta.schema}
      ...
      <Dialog open={!!editRow} onClose={() => setEditRow(null)} title={`Sua ${table}`} size="lg">
        {editRow && (
          <ZodForm
            schema={meta.schema}
